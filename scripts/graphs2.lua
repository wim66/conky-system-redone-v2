	...
	graph_settings = {
		{
			...
			nb_values=100,
			fg_bd_size = 2,
			fg_colour = {{0,0xFF0000,0.25},{0.5,0xFFA209,0.25},{1,0x00FF00,0.25}},
			fg_bd_colour = { {0,0xFF0000,1},{0.5,0xFFA209,1},{1,0x00FF00,1}},
			background = false,
		},

		{
			...
			height = 50,
			nb_values=50,
			fg_colour = {{0,0xFF0000,1},{0.5,0xFFA209,1},{1,0x00FF00,1}},
			fg_bd_colour = { {0,0xFF0000,1},{0.5,0xFFA209,1},{1,0x00FF00,1}},
			fg_bd_size = 2,
			background = false,
		},

		{
			...
			height = 50,
			nb_values=50,
			fg_colour = { {0,0xFF0000,1},{0.5,0xffff7f,1},{1,0x55aaff,1}},
			fg_bd_colour = { {0,0xFF0000,1},{0.5,0xffff7f,1},{1,0x55aaff,1}},
			fg_bd_size = 2,
			background = false,
		},
	}
...
function conky_draw_graph()
	...
	if updates == 1 then
		...
		for i in pairs(graph_settings) do
			if graph_settings[i].width == nil then
				graph_settings[i].width = 100
			end
	...
	if updates > updates_gap then
		for i in pairs(graph_settings) do
			if graph_settings[i].draw_me == true then
				...
				for j = 1, nb_values do
					...
					if j == nb_values then
						...
						if graph_settings[i].name == "" then
							...
							value = tonumber(
								conky_parse("${" .. graph_settings[i].name .. " " .. graph_settings[i].arg .. "}")
							)
						end
						graph_settings[i].values[nb_values] = value
					end
					graph_settings[i].automax = math.max(graph_settings[i].automax, graph_settings[i].values[j])
					--should stop weird glitches at beginning when no values reported yet for upspeed or diskio
					if graph_settings[i].automax == 0 then
...
function draw_graph(t)
	...
	local function rgb_to_r_g_b(colour)
		return ((colour[2] / 0x10000) % 0x100) / 255.,
			((colour[2] / 0x100) % 0x100) / 255.,
			(colour[2] % 0x100) / 255.,
			colour[3]
	end
	...
	if t.y == nil then
		t.y = conky_window.height - t.bg_bd_size
	end
	if t.bg_colour == nil then
		t.bg_colour = { { 0, 0x000000, 0.5 }, { 1, 0xFFFFFF, 0.5 } }
	end
	if t.bg_bd_colour == nil then
		t.bg_bd_colour = { { 1, 0xFFFFFF, 1 } }
	end
	...
	if t.fg_colour == nil then
		t.fg_colour = { { 0, 0x00FFFF, 1 }, { 1, 0x0000FF, 1 } }
	end

	if t.fg_bd_size == nil then
		t.fg_bd_size = 0
	end
	if t.fg_bd_colour == nil then
		t.fg_bd_colour = { { 1, 0xFFFF00, 1 } }
	end

	if t.autoscale == nil then
		t.autoscale = false
	end
	if t.inverse == nil then
		t.inverse = false
	end
	if t.angle == nil then
		t.angle = 0
	...
	if t.bg_bd_orientation == nil then
		t.bg_bd_orientation = "nn"
	end
	if t.bg_orientation == nil then
		t.bg_orientation = "nn"
	end
	if t.fg_bd_orientation == nil then
		t.fg_bd_orientation = "nn"
	end
	if t.fg_orientation == nil then
		t.fg_orientation = "nn"
	end
